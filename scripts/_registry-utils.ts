...
import { initialRegistryYamlSchema, resolvedRegistrySchema } from '@cybergogne/schemas';
...
export async function loadSeedYaml(filePath: string) {
  const raw = await readFile(filePath, 'utf8');
  return initialRegistryYamlSchema.parse(YAML.parse(raw));
}

export function hydratePublicationPolicy(record: any) {
  return {
...
export function hydrateScope(record: any) {
  return {
...
export async function loadResolvedYaml(filePath: string) {
  const raw = await readFile(filePath, 'utf8');
  return resolvedRegistrySchema.parse(YAML.parse(raw));
}
